...
That looks like something intersting might be going on.  Lets try plotting this parsed by the type of feed.
```{r}
...
Now that we know what our data looks like lets practice producing figures for a manuscript.  First we will use the `pdf` command this creates and opens the pdf file and that subsequent plotting commands are sent to.  Once are plot is complete then we use the `dev.off` command to close the file:

this will be our figure 1 that is the distribution of all chick weights at 6 weeks:
```{r}
pdf(file="fig1.pdf", width = 4, height = 4)
hist(chickwts$weight, 
     main = "Chick weights at 6 weeks", 
